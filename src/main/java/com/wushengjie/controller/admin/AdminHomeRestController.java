...
public class AdminHomeRestController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AdminHomeRestController.class);

    @Autowired
    private UserService userService;

    @RequestMapping(value={"/login"},method = RequestMethod.POST)
    public String login(@Valid User user, Model model, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        if (bindingResult.hasErrors()) {
            return "login";
        }
        String loginName = user.getLoginName();
        logger.info("准备登陆用户 => {}", loginName);
        UsernamePasswordToken token = new UsernamePasswordToken(loginName, Md5Util.pwdDigest(user.getPassword()));
        //获取当前的Subject
